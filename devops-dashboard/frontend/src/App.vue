<script setup>
import { ref, onMounted } from 'vue'
import { getHealth } from '@/services/health'
import { getCloudflareStatus } from '@/services/external'
import { getProviderStatus } from '@/services/providers'
import { getSystemMetrics } from '@/services/system'
import CpuRamChart from '@/components/CpuRamChart.vue'

const cpu = ref(null)
const ram = ref(null)
const uptime = ref(null)

async function loadSystem() {
  const data = await getSystemMetrics()
  if (!data.error) {
    cpu.value = data.cpu
    ram.value = data.ram_used
    uptime.value = data.uptime
  }
}

onMounted(() => {
  loadSystem()
  setInterval(loadSystem, 5000) // refresco cada 5s
})

// Estado proveedores globales
const providers = ref({})

// Datos reales desde Cloudflare (ya configurado por ti)
const cloudflareStatus = ref('Cargando...')

async function loadExternalSingle() {
  const data = await getCloudflareStatus()
  cloudflareStatus.value = `${data.provider}: ${data.status}`
}

// Estado global (Cloudflare, GitHub, Google)
async function loadProviders() {
  providers.value = await getProviderStatus()
}

// Estado salud servicios internos
const api = ref(false)
const db = ref(false)
const cache = ref(false)

// incidentes locales (pronto backend)
const incidents = ref([
  { id: 1, title: 'Errores en API', status: 'open' },
  { id: 2, title: 'Lentitud en BBDD', status: 'inprogress' },
  { id: 3, title: 'Jobs en cola', status: 'resolved' },
])

async function loadHealth() {
  const health = await getHealth()
  api.value = health.api
  db.value = health.db
  cache.value = health.cache
}

onMounted(() => {
  loadHealth()
  loadExternalSingle()
  loadProviders()

  setInterval(loadHealth, 5000)
  setInterval(loadExternalSingle, 60000)
  setInterval(loadProviders, 60000)
})

function createIncident() {
  alert('Crear incidente (pendiente backend)')
}

function formatStatus(st) {
  const map = {
    open: 'Open',
    inprogress: 'In Progress',
    resolved: 'Resolved',
  }
  return map[st]
}
</script>

<template>
  <div class="app-layout">
    <!-- Sidebar -->
    <aside class="sidebar">
      <h1>Observabilidad</h1>
      <nav class="primary-nav">
        <button class="nav-btn active">Dashboard</button>
        <button class="nav-btn">Servicios</button>
        <button class="nav-btn">Incidentes</button>
      </nav>
      <div class="sub-nav">
        <button class="nav-btn" @click="$router.push('/logs')">Logs</button>
        <span class="sub-link">Alertas</span>
      </div>
    </aside>

    <!-- Content -->
    <div class="content">
      <header class="topbar">
        <div class="env-badge prod">PRODUCCIÓN 🔴</div>
        <div class="env-badge qa">QA 🟡</div>
        <div class="env-badge dev">DEV 🟢</div>
        <div class="user">👤 Johnatan</div>
      </header>

      <main class="dashboard-container">
        <h1>Plataforma de Observabilidad</h1>
        <p class="subtitle">Monitoreo y gestión de servicios críticos</p>

        <!-- Estado interno -->
        <section>
          <h2>Service Status</h2>
          <div class="cards">
            <div class="card" :class="api ? 'ok' : 'warn'">
              <span>API {{ api ? '✅' : '❌' }}</span>
            </div>
            <div class="card" :class="db ? 'ok' : 'warn'">
              <span>DB {{ db ? '✅' : '❌' }}</span>
            </div>
            <div class="card" :class="cache ? 'ok' : 'warn'">
              <span>Cache {{ cache ? '✅' : '❌' }}</span>
            </div>
          </div>
        </section>

        <!-- Estado mundial -->
        <section>
          <h2>Estado Mundial</h2>
          <div class="cards">
            <div
              v-for="(status, name) in providers"
              :key="name"
              class="card"
              :class="
                status.includes('Operational') || status.includes('All Systems') ? 'ok' : 'warn'
              "
            >
              <span>{{ name }}: {{ status }}</span>
            </div>
          </div>
        </section>

        <!-- Incidentes -->
        <section>
          <h2>Incidentes</h2>
          <button class="btn" @click="createIncident">➕ Crear incidente</button>
          <ul class="incident-list">
            <li v-for="inc in incidents" :key="inc.id">
              <strong>{{ inc.title }}</strong> —
              <span class="status" :class="inc.status">{{ formatStatus(inc.status) }}</span>
            </li>
          </ul>
        </section>
      </main>
    </div>
  </div>
</template>

<style scoped>
/* Tu CSS intacto */
.app-layout {
  display: flex;
  min-height: 100vh;
  background: #f0f2f5;
  font-family: 'Inter', Arial, sans-serif;
}
.sidebar {
  width: 240px;
  background: #0b1220;
  color: white;
  padding: 28px 20px;
  display: flex;
  flex-direction: column;
  gap: 20px;
}
.primary-nav {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.nav-btn {
  padding: 10px;
  border: none;
  border-radius: 6px;
  background: transparent;
  color: #cbd5e1;
  cursor: pointer;
  text-align: left;
  transition: 0.2s;
}
.nav-btn:hover,
.nav-btn.active {
  background: rgba(255, 255, 255, 0.12);
  color: #fff;
}
.sub-nav {
  margin-top: 10px;
  display: flex;
  flex-direction: column;
  gap: 6px;
  font-size: 14px;
  color: #94a3b8;
}
.sub-link:hover {
  color: white;
}
.content {
  flex: 1;
  display: flex;
  flex-direction: column;
  background: #f8fafc;
}
.topbar {
  display: flex;
  gap: 12px;
  justify-content: flex-end;
  padding: 12px 20px;
  background: #ffffff;
  border-bottom: 1px solid #e2e8f0;
}
.dashboard-container {
  max-width: 1100px;
  margin: 0 auto;
  padding: 32px;
  margin-top: 20px;
}
.cards {
  display: grid;
  grid-template-columns: repeat(3, 180px);
  gap: 16px;
  margin-bottom: 28px;
}
.card {
  padding: 16px;
  border-radius: 10px;
  display: flex;
  justify-content: center;
  font-weight: 600;
  color: white;
}
.card.ok {
  background: #16a34a;
}
.card.warn {
  background: #eab308;
}
.btn {
  background: #2563eb;
  padding: 10px 18px;
  border-radius: 8px;
  border: none;
  color: white;
  cursor: pointer;
  margin-bottom: 20px;
}
.btn:hover {
  background: #1e40af;
}
.status.open {
  color: #dc2626;
}
.status.inprogress {
  color: #d97706;
}
.status.resolved {
  color: #16a34a;
}
.env-badge {
  font-size: 12px;
  padding: 5px 10px;
  border-radius: 6px;
  font-weight: bold;
}
.env-badge.prod {
  background: #fee2e2;
  color: #b91c1c;
}
.env-badge.qa {
  background: #fef9c3;
  color: #ca8a04;
}
.env-badge.dev {
  background: #dcfce7;
  color: #15803d;
}
.user {
  font-weight: 600;
  color: #1e293b;
  background: #e0f2fe;
  padding: 6px 10px;
  border-radius: 6px;
}
</style>
